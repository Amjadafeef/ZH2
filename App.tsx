...
const Footer: React.FC<{ t: Translation, lang: Language, content: Record<string, SiteContent>, onNavigate: (view: ViewState) => void }> = ({ t, lang, content, onNavigate }) => {
  const getTxt = (key: string) => getSafeTxt(content, key, lang);
  const getUrl = (key: string) => content[key]?.value_en || '#';
        ...
        <div className="grid md:grid-cols-12 gap-12 mb-20 border-b border-olive-800 pb-16">
          <div className="md:col-span-4">
            ...
            <div className="flex space-x-6 rtl:space-x-reverse">
              <a href={getUrl('social_facebook_url')} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-olive-800 flex items-center justify-center text-white hover:bg-gold-500 hover:text-white transition-all"><Facebook size={18} /></a>
              <a href={getUrl('social_instagram_url')} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-olive-800 flex items-center justify-center text-white hover:bg-gold-500 hover:text-white transition-all"><Instagram size={18} /></a>
            </div>
          </div>
...
const App: React.FC = () => {
  ...
  const [isContentMissing, setIsContentMissing] = useState(false);
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      {/* Warning Banner / CTA if DB is missing */}
      {isContentMissing && (
        <div className="fixed bottom-6 right-6 z-[100] animate-bounce">
          <button 
            onClick={() => setIsSetupOpen(true)} 
            className="bg-red-600 text-white px-6 py-4 rounded-full shadow-2xl font-bold flex items-center gap-3 hover:bg-red-700 transition-colors border-4 border-white"
          >
            <Database size={20} />
            <span>Fix Database Tables</span>
          </button>
        </div>
      )}
