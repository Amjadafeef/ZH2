...
import { 
  ...
  Trash2, Plus, Minus, Calendar, ArrowLeft, PlayCircle, Database, ExternalLink
} from 'lucide-react';
import { translations } from './constants';
import { Product, Feature, Testimonial, FAQItem, Language, Translation, SiteContent, CartItem, Article } from './types';
import { getProducts, getFeatures, getTestimonials, getFAQs, getSiteContent, getArticles, SETUP_SQL } from './services/supabaseService';
...
const HomeTestimonials: React.FC<{ t: Translation, lang: Language, content: Record<string, SiteContent>, onViewAll: () => void }> = ({ t, lang, content, onViewAll }) => {
...
const TestimonialsPage: React.FC<{ t: Translation, lang: Language, content: Record<string, SiteContent> }> = ({ t, lang, content }) => {
  const [testimonials, setTestimonials] = useState<Testimonial[]>([]);
  const [loading, setLoading] = useState(true);
    ...
    <div className="pt-32 pb-24 min-h-screen bg-olive-900 text-white">
       <div className="max-w-7xl mx-auto px-6 lg:px-8">
         <div className="text-center mb-16 animate-fade-in">
           ...
           <p className="text-olive-200 text-lg max-w-2xl mx-auto">{t.testimonials.subtitle}</p>
         </div>
         {loading ? (
    ...
    <div className={`min-h-screen flex flex-col font-sans ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      {view === 'home' && (
        <>
          ...
          <HomeTestimonials t={t} lang={lang} content={siteContent} onViewAll={() => setView('testimonials-index')} />
      ...
      {view === 'testimonials-index' && <TestimonialsPage t={t} lang={lang} content={siteContent} />}
