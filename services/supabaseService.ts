...
insert into public.testimonials (id, name_en, name_ar, text_en, text_ar, location_en, location_ar, sort_order, is_approved) values
('t400f1ee-6c54-4b01-90e6-d701748f0401', 'Sarah J.', 'سارة ج.', 'Best olive oil ever. The taste is incredibly fresh.', 'أفضل زيت زيتون. الطعم طازج للغاية.', 'Amman', 'عمان', 1, true),
('t400f1ee-6c54-4b01-90e6-d701748f0402', 'Mike T.', 'مايك ت.', 'Great packaging and fast delivery to Dubai.', 'تغليف رائع وتوصيل سريع إلى دبي.', 'Dubai', 'دبي', 2, true)
ON CONFLICT (id) DO NOTHING;
...
if (SUPABASE_URL && SUPABASE_ANON_KEY) {
  ...
  console.warn("⚠️ Supabase Keys missing! Data will not load. Check .env file.");
}

export const getSupabaseClient = () => supabase;

const fetchData = async <T>(table: string, orderBy?: string, limit?: number, filter?: { col: string, val: any }): Promise<T[]> => {
  if (!supabase) return [];
...
export const getTestimonials = async (): Promise<Testimonial[]> => {
  ...
  return fetchData<Testimonial>('testimonials', 'sort_order', undefined, { col: 'is_approved', val: true });
};

export const addTestimonial = async (review: NewTestimonial): Promise<{ success: boolean, error?: any }> => {
  if (!supabase) return { success: false, error: "No client" };

  // Insert the testimonial. 
  // Note: We populate both EN and AR fields with the same input for now to ensure it displays in both languages.
  const { error } = await supabase.from('testimonials').insert({
    name_en: review.name,
    name_ar: review.name,
    text_en: review.text,
    text_ar: review.text,
    location_en: review.location,
    location_ar: review.location,
    sort_order: 10, // Default low priority
    is_approved: false // Needs moderation by default
  });
