...
// ⚠️ NO FALLBACKS ALLOWED IN PRODUCTION
// If these are empty, the app will load in a "View Only" mode with no data,
// and the UI will prompt to check configuration.
const SUPABASE_URL = ENV_SUPABASE_URL;
const SUPABASE_ANON_KEY = ENV_SUPABASE_ANON_KEY;

// ==========================================
// 🛠️ DATABASE SETUP SQL (Safe to expose, just structure)
// ==========================================
...
const fetchData = async <T>(table: string, orderBy?: string, limit?: number): Promise<T[]> => {
  if (!supabase) {
    // Fail gracefully if no client (no env vars)
    return [];
  }
